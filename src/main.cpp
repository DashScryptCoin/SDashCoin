...
#if defined(NDEBUG)
# error "DashScoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xe9af858065f52812ea9528dcf2ae329dab5196882d44e0012cd271dc5b3d3c15");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // DashScoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "DashScoin Signed Message:\n";
...
{
    // DashScoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	if(nHeight == 1)
            {
            nSubsidy = 945000 * COIN; // 5% DASHS Premine for Bounties
   }
    else if (nHeight < 5000) { nSubsidy = 100 * COIN; } // 100 DASH REWARD 3.5 days 
    else if (nHeight < 100000) { nSubsidy = 10 * COIN; } // 10 DASH REWARD 2 year 
    else if (nHeight < 200000) { nSubsidy = 5 * COIN; } // 5 DASH REWARD 4 years

           else { nSubsidy = 2.5 * COIN; } // 2.5 DASH REWARD 

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 60; // DASHS: 1 Minute 
static const int64 nTargetSpacing = 60; // DASHS: 60 Seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // DashScoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // DashScoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe9af858065f52812ea9528dcf2ae329dab5196882d44e0012cd271dc5b3d3c15");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Hola! Aquí provam avam si va aixó";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("123484710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1489351140;
        ...
        assert(block.hashMerkleRoot == uint256("0x1cb2b7d48526cb7de26bf7da035e548e0d83811b4b2b2376e6a1287c29ef6678"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // DashScoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DashScoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DashScoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DashScoinMiner(CWallet *pwallet)
{
    printf("DashScoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("dashscoin-miner");
    ...
    try { loop {
        ...
        printf("Running DashScoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DashScoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DashScoinMiner, pwallet));
}
